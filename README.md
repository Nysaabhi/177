...
processFlightQuery(query) {
  ...
  // Flight pattern remains the same
          ...
          <div id="${id}-dropdown" class="city-dropdown">
            ${cities.map(city => `
              <div class="city-option" 
                ...
                ${city.charAt(0).toUpperCase() + city.slice(1)} (${airportData[city]})
              </div>
            `).join('')}
          </div>
        </div>
...
.city-dropdown::-webkit-scrollbar-thumb:hover {
  ...
  setTimeout(() => {
    const setupCitySearch = (inputId) => {
      ...
      options.forEach(option => {
        option.addEventListener('click', () => {
          const cityName = option.dataset.city;
          const cityCode = option.dataset.code;
          input.value = cityName.charAt(0).toUpperCase() + cityName.slice(1);
          hiddenInput.value = cityCode;
          dropdown.style.display = 'none';
          
          // Update selected state
          options.forEach(opt => opt.dataset.selected = 'false');
    ...
    setupCitySearch('from');
    setupCitySearch('to');

    // Trip type handler
    document.getElementById('tripType').addEventListener('change', (e) => {
      const returnDateGroup = document.getElementById('returnDateGroup');
      returnDateGroup.style.display = e.target.value === 'round-trip' ? 'block' : 'none';
    });
    ...
    document.getElementById('flightSearchForm').addEventListener('submit', (e) => {
      ...
      const url = this.generateFlightSearchUrl(
        formData.get('from'),
        formData.get('to'),
